...
def process(opts):
    ...
    output_file = args.output_path + '/cta_img/' + patientID + '_cta.nii.gz'
    label_dst = args.output_path + '/ane_seg/' + patientID + '_seg.nii.gz'
...
def run(args):
    ...
    if args.dicom_folder.split('/')[-1] == 'aneurysm1':
        ...
        #assert len(label_list) == 176

        # 以下几个多了一层文件夹
        label_list11 = glob.glob(os.path.join(args.dicom_folder, 'tongrenshiyiyuan', '*/*/*/*/*/*.nii.gz'))

        print ('label_list11:',len(label_list11))
    ...
    #with open(args.dicom_folder.split('/')[-1] + '_tonren.txt', 'w') as f:
        #for item in label_list:
            #f.write("%s\n" % item)

    for label in label_list11:
        print(label)
