from lib.medloaders import medical_image_process_brats as img_loader
...
def fix_seg_map(segmentation_map, dataset="iseg2017"):
    ...
    elif dataset == "brats2018" or dataset == "brats2019" or dataset == "brats2020" or dataset == "brats2021":
        ED = 2
        NCR = 1
        NET_NCR = 1
        ET = 3
...
def create_sub_volumes(*ls, dataset_name, mode, samples, full_vol_dim, crop_size, sub_vol_path, normalization='max_min',
    ...
    print ('total pos {}, total neg {}'.format(total_pos,total_neg))
    pos, neg = 0,0
    ...
    for i in range(samples):
        ...
        while True:
            ...
            if find_non_zero_labels_mask(full_segmentation_map, th_percent, crop_size, crop) and pos <=total_pos:
                ...
                break
            elif not find_non_zero_labels_mask(full_segmentation_map, th_percent, crop_size, crop) and neg <= total_neg:
                segmentation_map = img_loader.load_medical_image(label_path, type='label', crop_size=crop_size,
                                                                 crop=crop)
                segmentation_map = fix_seg_map(segmentation_map, dataset_name)
                for j in range(modalities - 1):
        ...
        filename = sub_vol_path + 'id_' + iD + '_sample_' + str(i) + '_modality_'
        list_saved_paths = []
        for j in range(modalities - 1):
        ...
        logging.info('total pos generate: {}, total neg patches generate: {}'.format(pos,neg))
...
def get_all_sub_volumes(*ls, dataset_name, mode, samples, full_vol_dim, crop_size, sub_vol_path,
    ...
    for vol_id in range(total):
        ...
        iD = ls[- 1][vol_id].split('/')[-2]
        filename = sub_vol_path + 'id_' + iD  +  '_modality_'
...
def generate_padded_subvolumes(full_volume, kernel_dim=(32, 32, 32)):
    ...
    assert x.size(2) % kw == 0
    assert x.size(1) % kh == 0
    assert x.size(0) % kc == 0
