...
model_list = ['UNET3D', 'DENSENET1', "UNET2D", 'DENSENET2', 'DENSENET3', 'HYPERDENSENET', "SKIPDENSENET3D",
              "DENSEVOXELNET", 'VNET', 'VNET2', "RESNET3DVAE", "RESNETMED3D", "COVIDNET1", "COVIDNET2", "CNN","HIGHRESNET"]


def create_model(args,config):
    model_name = config['train']['model']
    assert model_name in model_list
    optimizer_name = config['train']['opt']
    lr = config['train']['lr']
    in_channels = config['train']['inChannels']
    num_classes = config['train']['classes']
    ...
    if model_name == 'VNET2':
        ...
        model = ResNet3dVAE(in_channels=in_channels, classes=num_classes, dim=config['train']['dim'])
