...
class Trainer:
    ...
    def __init__(self, args, config, model, criterion, optimizer, train_data_loader,
        ...
        self.writer = TensorboardWriter(args,config)

        self.save_frequency = 1
        self.terminal_show_freq = self.config['train']['terminal_show_freq']
        self.start_epoch = 1

    def training(self):
        for epoch in range(self.start_epoch, self.config['train']['nEpochs']):
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, input_tuple in enumerate(self.train_data_loader):
            ...
            input_tensor, target = prepare_input(self.config,input_tuple=input_tuple, args=self.args)
