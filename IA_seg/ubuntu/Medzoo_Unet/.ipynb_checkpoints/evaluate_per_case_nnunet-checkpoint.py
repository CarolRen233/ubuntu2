...
parser.add_argument('--justhead', action='store_true', default=False, help='head cut evaluate')
...
def study_evaluate(config, gt_file_or_folder, pred_file_or_folder,pkl_filename, justhead, devices, pred_type='mask'):
    ...
    for i in range(len(gt_instances)):
        if gt_instances[i] in ['ExtA0009']:
            gt_instances.pop(i)
            pred_instances.pop(i)
    
    
    print ('len(gt_instances):',len(gt_instances) )
    print ('len(pred_instances):',len(pred_instances))
    print ('====gt_instances:=====\n',gt_instances[:5],gt_instances[-5:])
    print ('====pred_instances:=====\n',pred_instances[:5],pred_instances[-5:])
    ...
    for i, (gt_ins, pred_ins) in enumerate(zip(gt_instances, pred_instances)):
        ...
        if not justhead:
            ...
            utils.maybe_create_path(os.path.join(pred_file_or_folder,'full'))
            sitk.WriteImage(pred_itk_image, os.path.join(pred_file_or_folder,'full',ins_id+'_full.nii.gz'))
