...
ori_config = utils.load_config(os.path.join('./configs', args.config + '.yaml'))
args.logging_folder = args.pred_file_or_folder

def study_evaluate(args,config, gt_file_or_folder, pred_file_or_folder, devices, pred_type='mask'):
    ...
    print ('gt_instances:',len(gt_instances),gt_instances[:5])
    ...
    print ('pred_instances:',len(pred_instances),pred_instances[:5])
...
if __name__ == '__main__':
    ...
    print ('args.gt_file_or_folder:',args.gt_file_or_folder)
    print ('args.pred_file_or_folder:',args.pred_file_or_folder)

    logging.basicConfig(level=logging.INFO, filename=os.path.join(args.logging_folder,
                                                                  'log_'+ args.config+datetime.datetime.now().strftime(
                                                                      '%Y-%m-%d#%H-%M-%S') + '.txt'))
    ...
    study_evaluate(args,config, args.gt_file_or_folder,args.pred_file_or_folder, devices, pred_type)
