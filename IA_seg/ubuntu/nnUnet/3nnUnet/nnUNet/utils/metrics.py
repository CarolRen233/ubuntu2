...
class _ConfusionMatrix(torch.nn.Module):
    ...
    def __call__(self, y_pred, y_true, update=True, callback_fn=None, **kwargs):
        ...
        if y_pred.ndim == y_true.ndim:
            if y_true.ndim > 2:
                y_true = utils.transpose_move_to_end(y_true, 1)
        ...
        # y_pred to [-1,] shape
        if y_pred.ndim > 2:
            y_pred = utils.transpose_move_to_end(y_pred, 1)
...
class HD95(torch.nn.Module):
    ...
    def __call__(self, y_pred, y_true, **kwargs):
        ...
        if 'spacing' not in kwargs:
            logging.debug('no spacing specified.')
...
def per_target_transform(y_pred, y_true):
    ...
    per_target_preds = utils.one_hot(per_target_preds, 2, axis=1)
    per_target_trues = utils.one_hot(per_target_trues, 2, axis=1)
    return per_target_preds, per_target_trues


def get_evaluation_metric(config, device):
    """
    ...
    if any(['auc' in item for item in metrics]) or any(['ap' in item for item in metrics]) \
        ...
        if thresholds_type == 'logspace':
            ...
            logging.critical('thresholds_type is not supported: %s' % thresholds_type)
            exit(1)
    ...
    for metric_name in metrics:
        ...
        if metric_name.endswith('tp'):
            ...
            logging.error('Unrecognized metric: %s' % metric_name)
            continue
    for curve_name in curves:
        ...
        if curve_name.endswith('roc'):
            if curve_name.replace('roc', 'auc') not in metrics_dict:
                logging.warning('%s not in metrics but %s in curves. Adding %s to metrics'
    ...
    assert len(metrics_dict) != 0, 'No metric is added'
    print ('===metrics_dict===\n{}\n======curves_dict====\n{}'.format(metrics_dict,curves_dict))
