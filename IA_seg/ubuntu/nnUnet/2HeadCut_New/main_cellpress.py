...
def main():
    ...
    all_properties = load_pickle('./All_CellPressAll_info.pkl')
    ...
    predict_patients_names=[]
    for patient in patients_names:
        print (patient)
        if not os.path.exists(os.path.join(just_head_seg_save,patient+'.nii.gz')):
            predict_patients_names.append(patient)
        else:
            print (patient,' head cut exists!')
        
        
    ####===============test loader and model load=====================
    
    
    test_dataset = Dataset(predict_patients_names, input_2Dbone, classes=['brain'])
    ...
    save_pickle(all_properties, just_head_properties_save)
    shutil.copyfile('./after_headcut_properties_CellPressAll.pkl',os.path.join(args.save_dir, 'after_headcut_properties_CellPressAll.pkl'))
    f1 = open('resize_CellPressAll.txt', 'w')
...
def get_arguments():
    ...
    parser.add_argument('-i', '--original_dir', type=str, required=False, default='F:/data/CellPress1338',
                        help='the folder include cta_img and ane_seg')
    parser.add_argument('-csv', '--csv', type=str, required=False, default='./All_CellPressAll_info.csv',
                        help='ids')
    parser.add_argument('-o', '--save_dir', type=str, required=False, default='F:/data/CellPress1338/headcut_all',
                        help='output folder')
