...
def main():
    ...
    for testdata in test_loader:
        ...
        cta_nii = sitk.ReadImage(all_properties[name]['full_cta_file'])
        cta_np = sitk.GetArrayFromImage(cta_nii).astype(np.float32)
        seg_nii = sitk.ReadImage(all_properties[name]['full_seg_file'])
    ...
    save_pickle(all_properties, just_head_properties_save)
    shutil.copyfile('./after_headcut_properties.pkl',os.path.join(args.save_dir, 'after_headcut_properties.pkl'))
