...
parser.add_argument('-c', '--config', type=str, required=False, default='/root/workspace/renyan/code/GLIA-Net/configs',
...
parser.add_argument('-o', '--overwrite_config',  default=False)
...
if __name__ == '__main__':
    ...
    exp_path = args.config
    if not args.overwrite_config and os.path.exists(os.path.join(exp_path, 'GLIA-Net.yaml')):
        print ('----yaml path---:',os.path.join(exp_path, 'GLIA-Net.yaml'))
        config = load_config(os.path.join(exp_path, 'GLIA-Net.yaml'))
    else:
