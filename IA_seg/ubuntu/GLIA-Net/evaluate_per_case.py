...
def study_evaluate(config, gt_file_or_folder, pred_file_or_folder, logger, devices, pred_type='mask'):
    ...
    #problem_list = ['ExtA0009.nii.gz','ExtA0032.nii.gz','XJTr0151.nii.gz','XJTr0212.nii.gz','XJTr0265.nii.gz']

    '''
    total_gt = len(gt_instances)
    pop_list = []
    for i in range(total_gt):
        if os.path.basename(gt_instances[i]) in problem_list:
            pop_list.append(i)
    for j in pop_list:
        gt_instances.pop(j)
        pred_instances.pop(j)
    '''
